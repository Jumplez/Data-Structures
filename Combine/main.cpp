...
void readDataFromFileR(const string& filename, RBinarySearchTree& bst) {
    ...
    while (getline(infile, line)) {
        ...
        // Get the importart information from the file such as ->
...
// births count based Binary Search Tree
void readDataFromFileB(const string& filename, BBinarySearchTree& bst) {
    ifstream infile(filename);
    if (!infile.is_open()) {
        cerr << "Error opening file: " << filename << endl;
        return;
...
int main() {
    ...
    switch(dataStructure){
        case 1: {
            ...
            if (treeOption == 1) { // Region-based BST - Part (A)
            ...
            } else if (treeOption == 2) { // Births count-based BST
                BBinarySearchTree bst; // Create the object for births count-based data

                readDataFromFileB("Deaths_Births.txt", bst); // Load data from the file
                select_option = -1;
                while (select_option != 4) {
                    ...
                    switch (select_option) {
                        ...
                        case 3: {
                            bst.displayInOrder(); // Display the tree in order( the region with the lowest count of births is at the top of the tree, while the regions with the highest towards the bottom)
                            break;
                        }
                        case 4: {
                            cout << "Exiting...\n";
